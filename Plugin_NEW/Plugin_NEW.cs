        ...
        */
        ...
        private void DownloadImage(ImageData imageData)
        {
            ...
            */
            ...
            imageData.DownloadClient.GetAsync(imageData.FullAddress, imageData.CancellationToken).
                ContinueWith((x) =>
                {
                    if (x.IsCanceled || x.IsFaulted || !x.Result.IsSuccessStatusCode)
                       return;

                    imageData.Callback(imageData.Index, Image.FromStream(x.Result.Content.ReadAsStreamAsync().Result));
                });
